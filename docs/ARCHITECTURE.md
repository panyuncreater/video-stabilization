...
```
切换 ⟺ 帧间灰度 MAD > 25.0
     且（RANSAC 内点率 < 0.30 或 跟踪存活比例 < 0.25 或 探针存活比例 < 0.45）
     且 距上次切换 ≥ 12 帧
```
...
- **无状态探针（v2.3）**是第二层补丁：长期跟踪点集会退化为跨场景静态结构（存活 ≥30 永不重检测），到切换帧时存活率仅 ~0.55、内点率 0.8+，上述两线均无法触发（508/809/880 漏判根因）；故 MAD 候选帧上由 `probe_cut_evidence`（上一帧现检角点 + 单步跟踪的新鲜全集）独立取证，崩溃线 0.45（LK 残差 0.05 下实测：切换帧 0.099–0.237 vs 常态帧 0.656–1.000，取间隔中点），两路证据**取或**。仅候选帧触发，全片代价可忽略。注意：探针线与 LK 残差阈值存在交互（放宽至 0.075 会使切换帧探针存活抬升至 0.402，且实验证明 0.075 损害 ITF，已回退 0.05——见 KNOWN_ISSUES #19）。
...
实测：test1.mp4（1440 帧，1280×976）pass 1 ≈ 103 s、pass 2 ≈ 681 s（M1 自研特征/光流比 M0 cv2 慢约 3×，换来全链路自研）。
