# PROJECT_STATE
更新时间：2026-09-25（v2.5：根治「修复后比原视频更抖」）
当前阶段：**M2 验收达成（交付就绪）**——§11 全部 22 项验收基准通过（对照表见 docs/RESULTS.md 第五节）；**v2.5 角点空间均匀化已治愈感知回归**（KNOWN_ISSUES #21）；已批准三项后续优化（C 提速 → B 平滑器对比 → A 金字塔 LK，暂不执行，见「下一步」）；剩余交付为课程报告成稿
...
## 当前模块
- v2.5 角点空间均匀化已完成并验证；无进行中代码模块。

## 本次任务
- **v2.5「修复后比原视频更抖」根治（用户感知 → 选定方案 2：角点均匀化）**：用户反馈修复后视频反而更抖；根因定位为**帧间估计误差注入**——全局 Top-N 角点条带化使 RANSAC 逐帧平移误差 ε_t 经补偿矩阵 B_t 直接进入输出，而 ITF / 稳定度 S 对此不敏感（故既有验收全绿仍掩盖问题，KNOWN_ISSUES #21）。
- 修复：`src/features.py` Top-N 改为**近方形网格分桶**（桶数 ≥ max_corners、桶内取响应最大者），质量阈值 0.01→0.001（重检 0.005→0.0002）；`src/shots.py` 探针崩溃线 0.45→0.25 按新点集分布重标；新增空间均匀性单测。
- 决定性验证：以同法重跟踪 + 切换帧掩膜的二阶差分 RMS 对比原视频 / v2.4 / v2.5 三段输出——v2.4 垂直方向比原视频更抖 +8.3%（坐实用户感知），v2.5 相对原视频 −44.7%、相对 v2.4 −48.9%。
- 双份回归：合成不退化（S 0.8090 vs M0 0.8150，−0.74% ≤2%）；test1 四项达标且 ITF 增益 +0.687→+1.189（逼近 M0）、切换 4/4 检出、裁剪率 0.9685。§11 全部 22 项仍通过。

## 已知风险
- （已消除）切换假阴性、ITF 差距待决、覆盖率工具待批——三项遗留全部收口；v2.5 新增 #21（已解决）与 #22（非缺陷），KNOWN_ISSUES 无待定项。
- 探针崩溃线（v2.5 重标为 0.25）与 LK 残差阈值存在**参数交互**（放宽残差会使切换帧探针存活 0.146→0.402，判别间隔收窄）——后续若重调 LK 阈值必须重标探针线（shots.py docstring 已注明）。
- v2.5 均匀化使点集含更多弱角点，§12「存活 <30 重检测」触发次数由 2 升至 47，pass2 耗时 634→1055 s（指标不受影响，是「C. pass2 提速」优化要处理的对象）。
...
- M0 与 v2.5 的 ITF 差距（+1.299 vs +1.189）仍有残余，归因金字塔 LK，留作课程加分项（优化 A），报告局限性章节如实说明。
...
**已批准的优化方向（2026-09-25 用户选定 A/B/C 三项，暂不执行，待后续会话启动）**：

1. **B. 三平滑器对比实验**（性价比最高，报告核心素材）：ma / median / gauss 双份端到端对比 + `--window` 扫描（建议 15/31/61），零代码改动；预期证明高斯平滑与窗口选择的合理性，结果并入 docs/RESULTS.md 与报告「选型理由」章节。
2. **C. pass2 提速**（与 B 有协同，宜先行）：合并诊断 warp 与写出、按需关闭掩膜 ITF 诊断（KNOWN_ISSUES #17），预期 20–30%；B 需要多遍 test1 回归（每遍约 13 分钟），C 先落地可直接缩短 B 的实验周期。不影响任何指标。
3. **A. 金字塔 LK**（§8.2 加分项，改动最大，压轴实施）：test1 ITF 预期 +0.5~0.6 dB（+0.687→逼近 M0 的 +1.30，差距已归因金字塔）。实现要点：自研金字塔降采样（NumPy 向量化 2×2 均值池化；若涉及几何重采样一律走 `warp.resample`，红线 2）+ 粗到细 LK 迭代。实施时必须：重过 §8.2 全套验收（合成 EPE 现余量充足）、**重标探针崩溃线**（PROBE_SURVIVAL_THRESHOLD 与 LK 参数存在交互，见已知风险）、重跑双份回归。

执行顺序建议：**C → B → A**（提速先落地缩短实验周期，大改动压轴）；每项独立 commit。

**课程报告成稿**（最后交付项，用户主导、AI 协助整理）：素材已齐——数据结构五要素与实测耗时（docs/data_structures.md）、选型理由与架构（ARCHITECTURE.md）、实验数据（RESULTS.md 三列对比 + §11 验收对照表）、局限性讨论与已知问题（KNOWN_ISSUES）、「先报告数据再决策」案例（点集退化根治、0.075 证伪实验）。若 B 完成，三平滑器对比为报告增补关键素材。
