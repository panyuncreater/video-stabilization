# PROJECT_STATE
更新时间：2026-09-26（机上全量验收复跑：74 passed / 覆盖率 98% / 合成+实拍双档四项指标达标）
...
**机上验收复跑（2026-09-26，用户指令「测试一下文件夹里的项目」）——零代码改动，纯复跑取证**

环境（与锁定档的差异已逐项核对）：Python 3.10.11；项目内 venv 用 `--system-site-packages` 建立；
**pytest 9.1.1 / pytest-cov 7.1.0 / coverage 7.16.1 与 requirements 完全一致**；
**numpy 2.2.6、matplotlib 3.10.9 低于锁定**（要求 2.5.3 / 3.11.2——Python 3.10 无对应 wheel，
安装会转向源码编译，故沿用系统版本）；opencv-contrib-python 5.0.0.93（cv2 上报 5.0.0，与锁定同版）。

| 验收项 | 本次实测 | 文档基准 | 判定 |
|---|---|---|---|
| 红线静态自检（20 个主实现文件） | 无违规 | 无违规 | ✅ |
| pytest | **74 passed**（3.18 s） | 74 passed | ✅ |
| 核心模块覆盖率 | **98%**（heap/tracking 100%、smoothing/warp 98%、features 97%、motion/ring_buffer 95%） | 98%（线 ≥80%） | ✅ |
| 合成 ITF | 24.503 → 27.601（**+3.098**） | +3.098 | ✅ |
| 合成 S / 裁剪率 / D | 0.8091 / 0.9629 / 0.00677 | 0.8090 / 0.9629 / 0.00677 | ✅ |
| 实拍 ITF | 30.163 → 31.339（**+1.175**） | +1.189（实拍重复运行区间 +1.17~+1.20） | ✅ |
| 实拍 S / 裁剪率 / D | 0.9971（5 镜头）/ 0.9690 / 0.00187 | 0.9967 / 0.9685 / 0.00187 | ✅ |
| 切换检出 / 重检测 | **4/4 [508, 809, 880, 1263]** / 47 次 | 同 | ✅（与文档逐值一致） |
| 输出封装一致性 | 合成 960×540@30×200；test1 1280×976@24×1440，输入=输出 | 同 | ✅ |
| 可视化（输出到临时目录，不覆写 docs/） | 两个绘图函数均正常出图，中文无乱码、切换竖虚线正常 | — | ✅ |
| `make_synthetic.py` 复现性 | 重建 ground_truth 与入库版本数值一致（max\|diff\| ≤ 1.4e-14，仅浮点文本格式差异） | — | ✅ |

耗时（本机明显慢于文档档）：合成 pass1 34.0 s / pass2 95.2 s（文档 14.0 / 36.5 s）；
test1 pass1 290.6 s / pass2 2297.4 s（文档 98 / 1055 s）。**主因是复跑期间本机同时运行着外部计算任务
（3 个 `train_with_lm.py` 常驻约 185% CPU），CPU 争用导致 pass2 慢 2.2 倍；** 依赖版本与硬件差异为次因，
故此耗时不可与文档档直接比较。

**⚠ 本轮未执行**：`--vis` 直写 docs/（会覆写已入库的 4 张图，改为临时目录等价验证）、
`tools/profile_smoothers.py` 实拍档 9 配置（≈2.9 h）。
...
**本会话（2026-09-26 机上验收复跑，零代码/零口径改动）**：
- PROJECT_STATE.md（改）：本记录（环境差异、双档实测、未执行项）。
- **无源码、无测试、无文档数值改动**；复跑产生的临时产物一律未入库：`.venv/`（gitignore）、
  `output/`（gitignore）、`data/synthetic/synthetic_shaky.mp4`（gitignore）。
- 复跑中 `tools/bench_ds.py` 与 `tools/make_synthetic.py` 覆写了已入库产物
...
- 本会话（2026-09-26 机上复跑）验证：`tools/verify_env.py` 红线自检 **PASS**（20 个主实现文件无违规），
  但**判 exit 1（阻塞档）——唯一原因是 numpy 2.2.6 / matplotlib 3.10.9 低于锁定**，属环境而非工程问题。
  pytest **74 passed / 3.18 s**，覆盖率 **98%**。合成档 ITF +3.098 / S 0.8091 / 裁剪 0.9629 / D 0.00677 / 切换 0；
  实拍档 ITF +1.175 / S 0.9971（5 镜头）/ 裁剪 0.9690 / D 0.00187 / 切换 4/4 [508,809,880,1263] / 重检测 47；
  输出封装一致性双份 PASS；`bench_ds.py` 与可视化绘图函数（临时目录）均正常。
  **未执行**：`--vis` 直写 docs/、`profile_smoothers.py` 实拍档 9 配置。
- **文档过期项（本轮发现，未改，待确认）**：README §四、docs/TESTS.md、docs/RESULTS.md §四与 §五第 19 项
  仍写「60 passed」（RESULTS §五第 19 项另注「期望 64」），**实际为 74 passed**（P2 +4、P4 +10 未同步）。
